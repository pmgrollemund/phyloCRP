...
#' @param chain an integer, the MCMC chain index.
#' @export
#' @examples
...
compute_summaries <- function(posterior_sample,truth,chain=1){
 ...
 alpha_var  <- stats::var(posterior_sample$alpha[,chain])
 p_mean <- mean(posterior_sample$p[,chain])
 p_var  <- stats::var(posterior_sample$p[,chain])
 r_mean <- mean(posterior_sample$r[,chain])
 r_var  <- stats::var(posterior_sample$r[,chain])
 K_mean <- mean(posterior_sample$K_tilde[,chain])
 K_var  <- stats::var(posterior_sample$K_tilde[,chain])
 ...
 if(ncol(posterior_sample$c_tilde[,chain,]) < 10){
  ...
  c_tilde_mean <- c(apply(posterior_sample$c_tilde[,chain,],2,mean),
                    rep(0,add))
  c_tilde_var <- c(apply(posterior_sample$c_tilde[,chain,],2,stats::var),
                   rep(0,add))
 }else{
  c_tilde_mean <- apply(posterior_sample$c_tilde[,chain,1:10],2,mean)
  c_tilde_var <- apply(posterior_sample$c_tilde[,chain,1:10],2,stats::var)
 }
