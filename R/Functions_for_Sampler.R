...
rprior <- function(data,hyperparam){
 alpha <- rgamma(1,shape=hyperparam$lambda1,rate=hyperparam$lambda2)
 r     <- rnbinom(1,size=hyperparam$kappa,prob=1-hyperparam$pi)
 p     <- rbeta(1,hyperparam$eta1,hyperparam$eta2)
 N_tilde <- data$N + r
...
#' @param trace a list containing the \code{theta} values.
#' @param data see the example object \code{data1}
#' @param hyperparam see the example object \code{hyperparam1}
...
#' @export
#' @examples
#' res_MH <- MH(param1,data1,hyperparam1,verbose=F)
