...
MH <- function(param,data,hyperparam,verbose=TRUE){
 ## Initialization
 n_iter <- param$n_iter
 burnin <- param$burnin
 target <- param$target
...
PT <- function(param,data,hyperparam,verbose=TRUE){
 # Initialization
 n_iter <- param$n_iter
...
#'   \item{target}{(optional) a string, "posterior" (default) or "prior".}
#' }
#' @param datas a list of data, each item should be as the example object \code{data1}
#' @param hyperparam see the example object \code{hyperparam1}
#' @param verbose Write stuff.
...
PT_bs <- function(param,datas,hyperparam,verbose=TRUE){
 # Initialization
 n_iter <- param$n_iter
 temperature <- param$temperatures
 N <- length(temperature)
